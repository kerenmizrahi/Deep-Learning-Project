...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    N = len(text) // seq_len 
    samples = embedded_text[:N * seq_len].reshape(-1, seq_len, len(char_to_idx))
    labels_indices = torch.tensor([char_to_idx[char] for char in text[1:N*seq_len + 1]], dtype=torch.int64)
    labels = labels_indices.reshape(-1, seq_len)
    # ========================
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        ...
        for i in range(n_chars - len(start_sequence)):
            x = chars_to_onehot(out_text[-1], char_to_idx).unsqueeze(0).to(dtype=torch.float, device=device)
            y, h = model(x, h)
            y = hot_softmax(y[0, -1], temperature=T)
            idx = torch.multinomial(y, 1).item()
            out_text += idx_to_char[idx]
    # ========================
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        for i in range(0, len(self.dataset), self.batch_size):
            if i + self.batch_size > len(self.dataset):
                break
            idx.extend(range(i, i + self.batch_size))
        # ========================
        return iter(idx)
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for i in range(n_layers):
            ...
            self.add_module(f"wxz_{i}", wxz)
            self.add_module(f"whz_{i}", whz)
            self.add_module(f"wxr_{i}", wxr)
            self.add_module(f"whr_{i}", whr)
            self.add_module(f"wxg_{i}", wxg)
            self.add_module(f"whg_{i}", whg)
