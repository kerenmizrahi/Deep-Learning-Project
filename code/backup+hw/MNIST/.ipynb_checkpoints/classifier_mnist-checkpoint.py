...
import MNIST.autoencoder
#from MNIST.autoencoder import freeze_encoder
...
class Classifier(nn.Module):
    def __init__(self, input_size=128, hidden_layers=[64], output_size=10, dropout_rate=0.0):
        ...
        layers.append(nn.Linear(prev_size, output_size))  # (N, 10) - 10 classes (MNIST)
...
'''
    def tune_hyperparameters(self, hyperparameter_combinations, num_trials=10):
        best_acc = 0
        best_params = None

        for hparams in random.sample(hyperparameter_combinations, num_trials):
            trainer = clfTrainer(classifier, encoder, dl_train, dl_test,
                                 hyperparams, freeze_encoder, device)
            trainer.train_and_testClassifier()
            acc = train_and_testClassifier(hparams)
            if acc > best_acc:
                best_acc = acc
                best_params = hparams
        ...
        df = pd.DataFrame(results, columns=['learning_rate', 'batch_size', 'accuracy'])

        # Pivot DataFrame for heatmap
        heatmap_data = df.pivot(index='learning_rate', columns='batch_size', values='accuracy')
