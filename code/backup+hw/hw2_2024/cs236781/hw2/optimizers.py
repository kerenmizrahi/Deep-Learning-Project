...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            velocity = self.velocities[id(p)]
            velocity.mul_(self.momentum).sub_(self.learn_rate * dp)
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # TODO: Add your own initializations as needed.
        # ====== YOUR CODE: ======
        self.sq_grads = {id(p): torch.zeros_like(p) for p, _ in self.params}
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            sq_grad = self.sq_grads[id(p)]
            sq_grad.mul_(self.decay).add_((1 - self.decay) * dp**2)

            adjusted_lr = self.learn_rate / (torch.sqrt(sq_grad) + self.eps)
            p.sub_(adjusted_lr * dp)
            # ========================
