...
def cnn_experiment(
    ...
    bs_test=None, #batch size for testing
    batches=100,  #number of batches per epoch
    epochs=100,   #each epoch goes through all data 
    early_stopping=3,   #stops after 3 consecutive epochs if validation performance does not improve.
    checkpoints=None,
    ...
    layers_per_block=2,
    pool_every=2,
    hidden_dims=[1024],  #dim of hidden layers [dim_l1, dim_l2, ...]
    model_type="cnn",
    ...
    **kw,
    ...
    channels_per_block = [ layers_per_block * [num_filters] for num_filters in filters_per_layer ]
    channels = [ num_filters for block in channels_per_block for num_filters in block ]
    ...
    if model_type == "cnn" :
        ...
        model = model_cls(in_size = ds_train[0][0].shape, out_classses = 10, channels = channels, pool_every = pool_every, hidden_dims = hidden_dims, 
                        activation_type = "lrelu", pooling_params = dict(kernel_size = 2), **kw)
        kw.pop('conv_params')
    else:
